...
const PurchaseOrderList = () => {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        message.error(response.message || '加载采购单列表失败');
      }
    } catch (error) {
      message.error(error.message || '加载采购单列表失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = async (orderId) => {
    try {
      const response = await inventoryService.getPurchaseOrder(orderId);
      if (response.success) {
        ...
        form.setFieldsValue({
          ...
          expectedDate: response.data.expectedDate ? dayjs(response.data.expectedDate) : null,
        });
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingOrder) {
        const response = await inventoryService.updatePurchaseOrder(editingOrder.id, data);
        if (response.success) {
        ...
        const response = await inventoryService.createPurchaseOrder(data);
        if (response.success) {
  ...
  const handleComplete = async (id) => {
    try {
      const response = await inventoryService.completePurchaseOrder(id, true);
      if (response.success) {
        message.success('采购单已完成，并已自动创建入库单');
        loadOrders();
      } else {
        message.error(response.message || '完成采购单失败');
      }
    } catch (error) {
      message.error(error.message || '完成采购单失败');
    }
  };

  const handleBatchDelete = async (keys) => {
    try {
      await Promise.all(keys.map(id => inventoryService.deletePurchaseOrder(id)));
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          ...
          {record.status === 'pending' && (
            <Popconfirm
              title="确定要完成这个采购单吗？完成后将自动创建入库单并更新库存"
    ...
    >
      ...
      >
        ...
        <div style={{ marginTop: 24 }}>
          <div style={{ marginBottom: 16, fontWeight: 'bold' }}>采购明细</div>
          <Table
