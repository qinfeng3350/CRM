...
const InboundOrderList = () => {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      const response = await inventoryService.getInboundOrders(params);
      if (response.success) {
  ...
  const handleEdit = async (orderId) => {
    try {
      const response = await inventoryService.getInboundOrder(orderId);
      if (response.success) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingOrder) {
        const response = await inventoryService.updateInboundOrder(editingOrder.id, data);
        if (response.success) {
        ...
        const response = await inventoryService.createInboundOrder(data);
        if (response.success) {
  ...
  const handleComplete = async (id) => {
    try {
      const response = await inventoryService.completeInboundOrder(id);
      if (response.success) {
        message.success('入库单已完成');
        loadOrders();
      } else {
        message.error(response.message || '完成入库单失败');
      }
    } catch (error) {
      message.error(error.message || '完成入库单失败');
    }
  };

  const handleBatchDelete = async (keys) => {
    try {
      await Promise.all(keys.map(id => inventoryService.deleteInboundOrder(id)));
  ...
  const columns = [
    ...
    { title: '入库单号', dataIndex: 'orderNumber', key: 'orderNumber' },
    ...
    >
      ...
      >
        ...
        <div style={{ marginTop: 24 }}>
          <div style={{ marginBottom: 16, fontWeight: 'bold' }}>入库明细</div>
          <Table
